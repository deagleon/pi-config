...
**⚠️ DEFAULT: DISABLED** - Use `/bash-guard on` to enable.

## Quick Start

```bash
/bash-guard status   # Check current status
/bash-guard on      # Enable protection
/bash-guard off     # Disable protection
```
...
### Subagent (`PI_SUBAGENT_DEPTH` ≥ 1) — headless hard-block (when enabled)
...
- State is persisted in `~/.pi/agent/extensions/bash-guard/state.json`.