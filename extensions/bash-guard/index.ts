...
// =============================================================================
// State Management (enabled by default = OFF)
// =============================================================================

const STATE_FILE = join(homedir(), ".pi", "agent", "extensions", "bash-guard", "state.json");
...
function saveState(state: GuardState): void {
	try {
		...
		const dir = join(homedir(), ".pi", "agent", "extensions", "bash-guard");
...
export default function (pi: ExtensionAPI) {
	// Register command to enable/disable bash-guard
	pi.registerCommand("bash-guard", {
		description: "Enable or disable bash-guard protection. Use: /bash-guard on | off | status",
		handler: async (args, ctx) => {
			const state = loadState();
			const subcommand = args.trim().toLowerCase();

			if (subcommand === "status" || subcommand === "" || subcommand === "s") {
				if (state.enabled) {
					ctx.ui.notify("🟢 bash-guard: ENABLED - commands will be checked for safety", "info");
				} else {
					ctx.ui.notify("⚪ bash-guard: DISABLED - use /bash-guard on to enable protection", "info");
				}
				return;
			}

			if (subcommand === "on" || subcommand === "enable" || subcommand === "1" || subcommand === "true") {
				state.enabled = true;
				saveState(state);
				ctx.ui.notify("🟢 bash-guard ENABLED - commands will be checked for safety", "success");
				return;
			}

			if (subcommand === "off" || subcommand === "disable" || subcommand === "0" || subcommand === "false") {
				state.enabled = false;
				saveState(state);
				ctx.ui.notify("⚪ bash-guard DISABLED - all commands pass through", "info");
				return;
			}
	...
	// If disabled, just show status and return early
	if (!state.enabled) {
		return;
	}
	...
	if (_isSubagent) {
		// Subagent mode: hard-block catastrophic operations
